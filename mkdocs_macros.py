...
def get_django_settings():
    ...
    if _django_settings is None:
        ...
        # Mock out imports
        sys.modules.update({'django.utils.safestring': Mock(mark_safe=lambda s: s),
...
def get_constance_config_type(default, type_hint=None):
    ...
    for choice_field in ('asset_bitrate_choices', 'asset_encoding_choices', 'autodj_requests_choices'):
        choices = settings.CONSTANCE_ADDITIONAL_FIELDS[choice_field][1]['choices']
        type_hints_to_string[choice_field] = f'Choice of: {", ".join(choice for _, choice in choices)}'
