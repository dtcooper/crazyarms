...
class AudioAsset(AudioAssetBase):
    ...
    def get_next_for_autodj(cls, run_with_playlist=True, run_no_repeat_artists=True, run_no_repeat_track_ids=True):
        ...
        if audio_asset is not None:
            ...
            logger.info(f'selected {audio_asset} '
    ...
    def get_next_for_autodj(cls, now=None):
        ...
        if current:
            ...
            while rotator_and_asset_list:
                ...
                if asset:
                    logger.info(f'Picked asset {asset} from rotator {rotator} in stopset {stopset}. '
                                f'{len(rotator_and_asset_list)} left to generate.')
                    break

                else:
                    logger.warning(f'Rotator {rotator} in {stopset} has no asset. '
                                   f'{len(rotator_and_asset_list)} left to generate.')

            if rotator_and_asset_list:
                cache.set(constants.CACHE_KEY_AUTODJ_CURRENT_STOPSET,
                          (stopset, rotator_and_asset_list), timeout=STOPSET_CACHE_TIMEOUT)
            else:
                ...
                cache.set(constants.CACHE_KEY_AUTODJ_STOPSET_LAST_FINISHED_AT,
                          finished_at, timeout=STOPSET_CACHE_TIMEOUT)
