...
class AntiRepeatTests(TestCase):
    ...
    @override_config(AUTODJ_ANTI_REPEAT_NUM_TRACKS_NO_REPEAT=5, AUTODJ_ANTI_REPEAT_NUM_TRACKS_NO_REPEAT_ARTIST=3)
    def test_basic_no_repeat(self):
        ...
        self.assertEqual(self.get_no_repeat_artists(), [AudioAsset.normalize_artist(f'A:{a}') for a in (2, 1, 0)])

    @override_config(AUTODJ_ANTI_REPEAT_NUM_TRACKS_NO_REPEAT=0, AUTODJ_ANTI_REPEAT_NUM_TRACKS_NO_REPEAT_ARTIST=0)
    ...
    @override_config(AUTODJ_ANTI_REPEAT_NUM_TRACKS_NO_REPEAT=0, AUTODJ_ANTI_REPEAT_NUM_TRACKS_NO_REPEAT_ARTIST=2)
    def test_no_artist_repeats_only(self):
        ...
        self.assertEqual(played_track_names, ['A:0 - T:0', 'A:1 - T:2', 'A:2 - T:4', 'A:0 - T:0', 'A:1 - T:2'])

    @override_config(AUTODJ_ANTI_REPEAT_NUM_TRACKS_NO_REPEAT=2, AUTODJ_ANTI_REPEAT_NUM_TRACKS_NO_REPEAT_ARTIST=3)
    def test_no_track_repeats_only(self):
        self.create_assets(3, 1)
        played_track_names = [str(AudioAsset.get_next_for_autodj()) for _ in range(5)]
        self.assertEqual(played_track_names, ['A:0 - T:0', 'A:0 - T:1', 'A:0 - T:2', 'A:0 - T:0', 'A:0 - T:1'])
    ...
    def test_corner_cases_when_anti_repeat_not_possible(self):
        ...
        with self.assertLogs('carb.autodj.models', level='INFO') as logger:
            self.assertIsNone(AudioAsset.get_next_for_autodj())
        self.assertEqual(logger.output, ['WARNING:carb.autodj.models:autodj: no assets exist, giving up early'])
