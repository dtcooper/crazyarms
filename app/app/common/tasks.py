...
def install_youtube_dl():
    if not (cache.get(constants.CACHE_KEY_YTDL_UP2DATE) and shutil.which(YOUTUBE_DL_CMD)):
        ...
        subprocess.run(['pip', 'install', '--no-cache-dir', '--upgrade', YOUTUBE_DL_PKG], check=True,
...
def asset_download_external_url(asset, url, title='', task=None):
    ...
    try:
        args = [YOUTUBE_DL_CMD, '--newline', '--extract-audio', '--no-playlist', '--max-downloads', '1',
                '--no-continue', '--audio-format', config.ASSET_ENCODING, '--exec', 'echo {}',
                '--socket-timeout', '10', '--output', f'{PROCESSING_TEMP_PREFIX}/%(title)s.%(ext)s']
            ...
            if cmd.returncode == 0:
                ...
                for field in asset.TITLE_FIELDS:
                    if not getattr(asset, field):
                        setattr(asset, field, next((
                            # The first youtube-dl field that matches the list YOUTUBE_DL_TITLE_FIELD_MAPPINGS or blank
                            json_data.get(ytdl_field) for ytdl_field in YOUTUBE_DL_TITLE_FIELD_MAPPINGS[field]
