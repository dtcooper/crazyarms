    ...
    def render_conf(self):
        ...
        if settings.ZOOM_ENABLED:
            ...
            self.render_supervisor_conf_file(
                command=f'sh -c "wait-for-it -t 0 localhost:4713 && {liq_cmd}"',
                **kwargs,
            )
            self.render_supervisor_conf_file(
...
class UpstreamService(ServiceBase):
    ...
    def render_conf(self):
        ...
        for upstream in UpstreamServer.objects.all():
            self.render_conf_file(
                "upstream.liq",
                conf_filename=f"{upstream.name}.liq",
                context={"upstream": upstream},
            )
            self.render_supervisor_conf_file(
                command=f"liquidsoap /config/upstream/{upstream.name}.liq",
                program_name=upstream.name,
                user="liquidsoap",
            )
...
class ZoomService(ServiceBase):
    ...
    def render_conf(self):
        kwargs = {
            "environment": f'TZ="{settings.TIME_ZONE}",HOME="/home/user",DISPLAY=":0",PULSE_SERVER="harbor"',
            "user": "user",
        }
        ...
        self.render_supervisor_conf_file(
            ...
            command=(
                "xvfb-run --auth-file=/home/user/.Xauthority --server-num=0 "
                "--server-args='-screen 0 1250x875x16' icewm-session"
            ),
            **kwargs,
        )
        self.render_supervisor_conf_file(program_name="x11vnc", command="x11vnc -shared -forever -nopw", **kwargs)
        self.render_supervisor_conf_file(
            program_name="websockify",
            command="websockify 0.0.0.0:6080 localhost:5900",
            **kwargs,
        )
        ...
        self.render_supervisor_conf_file(
            program_name="zoom",
            command='sh -c "killall -q zoom; zoom"',
            start=False,
            **kwargs,
        )
