...
class UpstreamServer(models.Model):
    ...
    hostname = models.CharField("hostname", max_length=255, help_text="Hostname for the server, eg. example.com")
    ...
    bitrate = models.PositiveSmallIntegerField(
        ...
        null=True,
        blank=True,
        help_text="Encoding bitrate (kbits), blank for a sane default or ffmpeg.",
    )
    mime = models.CharField(
        "MIME format",
        max_length=50,
        help_text="MIME format, ie audio/mpeg, leave blank for Liquidsoap to guess. (Needed for ffmpeg.)",
        blank=True,
    )
    encoding_args = models.JSONField(
        ...
        help_text=mark_safe(
            ...
            "Enter any additional arguments for the encoder here. Advanced use cases only, see the <a"
            ' href="https://www.liquidsoap.info/doc-1.4.3/encoding_formats.html" target="_blank">Liquidsoap'
